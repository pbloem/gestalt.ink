...
Note two key properties. First, the shape of the input is the same as the shape of the output. If you've only seen [basic neural networks](/neural-networks) so far, you can think of both the input and output as a vector of $n$ numbers.

<aside>If you've seen convolutions and tensors, you can also think of the in and output as a tensor representing, for instance, an image. Then the input and output are any kind of tensor, but the input and output tensor must have the same shape. We'll assume that the input and output are vectors for now.
</aside>

Second, somewhere between the input and the output, there is a hidden layer if size $m$, where $m$ is substantially smaller than $n$. This point in the network is called the _bottleneck_.
